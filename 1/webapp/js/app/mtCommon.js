...
(function($) {
	$.myAjax = function(ajaxData) {
		...
		try {
			...
			// 判断是否登录过
			if (!sessionStorage.getItem('token')) { // 未登陆过，登录
				if (name && pwd) {
					autoLogin(name,pwd);
				} else { // 用户未登录过
					setTimeout(function() {
			...
			} else if(Date.parse(new Date()) - sessionStorage.getItem('tokenUtime') > 900000) {
				autoLogin(name,pwd);
			}
			/* **自动登录-end** */
		} catch (err) {
...
function getToken(url) {
	if (!sessionStorage.getItem('token')) {
		...
		sessionStorage.setItem('tokenUtime', Date.parse(new Date()));
		if (url.substring(url.length - 3, url.length) == '.do') {
			return url + '?token=' + session.substring(0,session.length-2) + '&tokenType='+session.substring(session.length-1,session.length);
		} else {
			return url + '&token=' + session.substring(0,session.length-2) + '&tokenType='+session.substring(session.length-1,session.length);
		}
	}
	...
	$.ajax({
		...
		success: function(result) {
			if (result.flag) {
				...
				if(result.msg.indexOf("用户名或密码错误")>=0){
					window.location.href = '../index/login.html';
				} else {
...
function myConfirm(message, YES, NO) { 
	if(!!NO && (typeof NO === 'string')){ // ‘NO’如果是字符串，则不显示取消按钮
